...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Create default copy from Letter version
        Copy-Item "$outputDir/solid-ai-whitepaper-letter.pdf" "$outputDir/solid-ai-whitepaper.pdf" -Force
        Write-Host "   Updated default solid-ai-whitepaper.pdf" -ForegroundColor Gray
    } else {
